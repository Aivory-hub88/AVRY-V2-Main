...
7. **🔴 Blocking, decide first:** resolve the host-agent-alias mapping (§3/§6) — how does `X-Agent-Type` (`finance_invoice_ops`, `office_assistant`, `customer_service`, `leads_qualifier`, `autonomous`) select a Cerveau `[agents.<alias>]` entry? Nothing below can proceed on a live agent until this is answered; it's a product/Phase-6 decision, not something to invent unilaterally in config.
8. **Wire a real Composio toolkit connection** per agent type once (7) is answered — start with `finance_invoice_ops` (Stripe or QuickBooks first, see `invoice-processing`'s reference doc) since it has the most groundwork already.
9. **Install all four skills** into their resolved host agents once (7)/(8) land, E2E-test each against a real connection.
10. **Then choose:** continue Phase 4 (4.2 capability graph / 4.3 approval tiering) vs. finishing the deferred durability/memory items below (§6).
...
- **Host-agent-alias mapping undecided (found 2026-07-20):** Cerveau's `:3100` `[agents.<alias>]` entries are still vanilla zeroclaw's 6 generic brain roles; webhook host-agent selection (`?agent=` query param) is independent of the `X-Agent-Type` tenant header. No document specifies how Aivory's 5 agent types should map to a host alias. Blocks installing any of the 4 drafted skills to a live, traffic-serving agent. See `cerveau-skills/README.md` for the full technical trace.
