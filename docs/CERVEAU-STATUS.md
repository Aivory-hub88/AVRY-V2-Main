...
Verified live E2E: two data-row-only tenants adopt distinct personas; cross-tenant memory/knowledge blindness; tenant-scoped `memory_store` writes a real row in `cerveau.memories`. Throwaway `cerveau_e2e` DB dropped.
...
- **Phase 4** 🟡 started — **4.1 slice 1 landed (patch 0010):** tenant-entity-scoped MCP server resolution primitive (`McpServerConfig.tenant_entity_query_param` + `Config::mcp_servers_for_agent_and_tenant`), so a Composio-as-MCP server can be entity-scoped to the authenticated tenant's platform user id, never a shared/default account. Also: two Aivory-authored skills drafted in `~/Documents/AVRY-Cerveau/cerveau-skills/` (parse-verified via `SkillDocument::parse`, neither installed to a running instance yet) — `finance-invoice-ops/invoice-processing/` (jurisdiction-agnostic invoice extraction; 7 Composio accounting toolkits confirmed enabled on Aivory's account: QuickBooks, Xero, Stripe, Zoho Books, FreshBooks, Sage, NetSuite) and `office-assistant/meeting-outcomes/` (meeting → decisions/action-items/risks, synced to Notion/Slack/Sheets via toolkits already curated — found and fixed a live bug in the process, `COMPOSIO_CURATED.slack` pointed to Composio's deprecated `SLACK_CHAT_POST_MESSAGE`). **Not yet done:** actually wiring a toolkit connection live, capability graph (4.2), approval tiering (4.3), concurrent tools. Includes the [Agents-page integrations UX note](DEPLOYABLE_AGENT_RUNTIME_PLANNING.md) (tabbed Identity|Connections|Tools; tool-scope backend ships with the toggle, never before).
...
4. **Verify 0008 live** — turn on `vector_enabled=true` on a **throwaway schema** (not `cerveau`), confirm no "runtime within runtime" crash-loop, then turn it back off until the pgvector extension is actually installed (§6 — code fix alone doesn't help without the extension).
5. **Wire a real Composio accounting toolkit** for `finance_invoice_ops` — Stripe or QuickBooks first (see the invoice-processing skill's `references/composio-toolkits.md` for the full checklist + suggested order), then install the skill into that agent's skill bundle.
6. **Install + E2E-test `meeting-outcomes`** for `office_assistant` against a real Notion/Slack/Sheets connection.
7. **Deploy the `telegram-agent.js` Slack-tool fix** (`AVRY-V2-Main` commit `fc7a765`, swaps deprecated `SLACK_CHAT_POST_MESSAGE` → `SLACK_SEND_MESSAGE`) to the live vps-bridge process — pushed to git 2026-07-19, not yet live.
8. **Then choose:** continue Phase 4 (4.2 capability graph / 4.3 approval tiering) vs. finishing the deferred durability/memory items below (§6).
...
Still not rotated as of 2026-07-19. Growing more overdue, not less — the
GitHub PAT in particular has now transited multiple sessions and been reused
repeatedly (pushes, CI API polling/cancel/rerun) since first flagged.

- **GitHub PAT** `ghp_XIpM…` — replace with a fine-grained PAT scoped to only the `AVRY-Cerveau` repo. Used repeatedly across 2026-07-18/19 for pushes and GitHub Actions API calls (poll/cancel/rerun); every use is another transcript exposure.
- **`avry-postgres` password** (`aivory` user) — contains `@#`, visible in transcript.
- **Composio API key** `ak_I7jFXNZ…` — pasted directly in chat 2026-07-19 to check toolkit availability (`GET /api/v3/toolkits/<slug>`, read-only). Not stored in any file, but it transited the transcript — rotate it too.
