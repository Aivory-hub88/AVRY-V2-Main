...
## 12. Vanilla zeroclaw — shared graph, backported from source (not a binary swap)

**The gap.** Vanilla zeroclaw (`zeroclaw.service`, `:3010`) serves Console/Assistant/Workflow-builder in production. It is a genuinely different binary from `zeroclaw-cerveau` (confirmed: same base `0.8.4` release, but the Cerveau one carries 39+ patches vanilla never got — tenant multi-tenancy, rate limiting, MCP scoping, memory lifecycle, and more). Two ways to give it graph memory: swap it to the Cerveau binary (fast — the binary already exists — but every one of those 39+ patches rides along, none of them exercised on vanilla's actual usage pattern), or backport just the graph tools onto vanilla's own source (slower, but vanilla's existing behavior stays byte-for-byte the same for everything else). Asked directly; chose the backport.

**Fork:** `github.com/Aivory-hub88/zeroclaw` (from `zeroclaw-labs/zeroclaw`, tag `v0.8.4` — the exact base both this and the Cerveau fork started from), branch `cerveau-cognee-vanilla`. First commit `3e93d96`.

**Shared graph, not per-brain — the product decision from the earlier discussion.** Vanilla has no tenant concept at all (static `[agents.<alias>]` config identities, no dynamic provisioning), so there's no natural per-request identity to derive isolation from the way the Cerveau fork does. Rather than invent one, every brain (`workflow_brain`, `diagnostic_brain`, `analyst_brain`, ...) sends the SAME fixed `X-Tenant-Id`/`X-Agent-Type` pair (`[cognee].vanilla_agent_id`/`vanilla_agent_type`, both configurable, defaulting to values chosen not to collide with a real Cerveau tenant alias on the same shared sidecar). Reasoning: these are Aivory's own internal brains, not isolated customer data — `workflow_brain` benefiting from something `diagnostic_brain` learned is the point.

**What changed, kept deliberately small:**
- `[cognee]` config section (`enabled`, `base_url`, `internal_secret`, `vanilla_agent_id`, `vanilla_agent_type`) — same shape as the Cerveau fork's version, no tenant fields since there's nothing to thread.
- `graph_memory.rs` — same `graph_remember`/`graph_recall` tool pair, simpler than the Cerveau version: no `zeroclaw-tools`-can't-see-`zeroclaw-runtime` problem to solve here, because there's no per-call identity to resolve in the first place — the fixed config values are read directly inside the tool.
...
**The premise this section corrects.** "Wire Workflow Copilot to the graph" sounds like it should route through zeroclaw's `workflow_brain` — it doesn't, and never will under §12's design. Traced the actual call path: `avry-user-dashboard`'s `app/api/workflows/copilot/route.ts` → `bridgeCopilot.ts` → `POST {VPS_BRIDGE_URL}/console/stream` with `entrypoint: 'workflow_clarify'` (or `workflow_generate`/`repair`/`edit`/`semantic_review`) → `vps-bridge`'s `handleConsoleRouter` in `server.js`, which routes EVERY `workflow_*` entrypoint straight to `handleWorkflowClarifyDirect`/`handleWorkflowCopilotDirect` — direct OpenRouter calls, zero tools, zeroclaw never touched. This is a deliberate, well-documented 2026-08-20/22 fix (see `server.js`'s own comments around `WORKFLOW_CLARIFY_INSTR`): routing these through zeroclaw meant every clarify turn paid for `agent_analyst_brain`'s full ~50-tool, ~98k-token risk profile, which caused unreliable replies (wrong language, invented-word gibberish, leaked prompt fragments) — confirmed via runtime trace, not guessed. So §12's vanilla backport has **zero effect on Workflow Copilot**, regardless of which brain the tools are wired to.

**The fix: log to the graph from the dashboard's own Next.js server, bypassing both zeroclaw AND vps-bridge.** `app/api/workflows/activate/route.ts` already runs server-side (Next.js `runtime = 'nodejs'`) on the same box as the cognee-rs sidecar, and already has `host.docker.internal` wired (`extra_hosts`). No need to touch `vps-bridge/server.js` at all — genuinely a different, simpler integration point than either zeroclaw path.

**Where NOT to hook in, and why that mattered.** The obvious naive move — call `graph_recall` during clarify to give the copilot context, or `graph_remember` right after generation — would inject an HTTP round-trip into the exact hot path that was just de-tooled for latency and reliability reasons. Doing that would risk reintroducing the class of problem the 2026-08-20/22 fix closed. Instead: a single fire-and-forget `graph_remember` call, fired once **after a deploy to n8n has already succeeded** (`POST /api/workflows/activate`'s success path) — never awaited for response ordering, a sidecar hiccup here is a console warning, not something the caller has to handle, because the deploy already succeeded by the time this code runs.

**New file `lib/workflows/cogneeGraphLog.ts`**, wired into `app/api/workflows/activate/route.ts`. Own identity (`aivory_workflow_copilot`/`workflow_copilot`), separate from vanilla zeroclaw's shared identity — different kind of institutional knowledge (which integrations/workflow shapes actually got deployed, vs. general Console/Assistant/brain knowledge), not because it needs isolation from it. New `COGNEE_BASE_URL`/`COGNEE_INTERNAL_SECRET` env vars on the `avry-user-dashboard` container; the feature is a no-op (skips silently) when the secret isn't configured.

**Two real bugs found and fixed while verifying this live, neither in the new code:**

1. **`ufw` had no rule for port 3200.** `:3100`/`:3101` (Cerveau) each had three explicit allow rules (`127.0.0.1`, the host's own IP, `172.16.0.0/12` for the Docker bridge range) — port `3200` had none, added when this integration was built, so it was never opened for any container traffic until now. `avry-backend` reaching `:3200` in earlier sections of this ADR worked only because those tests ran `docker exec` *inside* the container's own network namespace in ways that happened not to need the host firewall in the same way, or were run before this specific check surfaced it — the dashboard container's connection attempt (a plain `fetch` from a different container) was the first to actually exercise the gap. Fixed: added the identical three-rule pattern for `3200`.
2. **`cognee-cerveau`'s Postgres migrations run once at process startup, not lazily per-request.** A `DROP SCHEMA cognee CASCADE` cleanup (done as part of an earlier live-verification pass in this same ADR) left the schema empty; the running process kept believing it was already migrated and only found out otherwise when a real `cognify`/`search` call hit `relation "graph_edge" does not exist`. Health check briefly showed `"unhealthy"` for the same reason. Fixed operationally (not a code bug): `systemctl restart cognee-cerveau.service` re-runs migrations against the current (now correctly recreated) schema. **Operational lesson for this project going forward: any future `DROP SCHEMA cognee CASCADE` cleanup must be immediately followed by a service restart**, not just a `CREATE SCHEMA` — recorded here so it isn't rediscovered the hard way again.

**Live-verified, from inside the actual `avry-user-dashboard` container** (not a workaround, the real network path the feature uses): `add` → `cognify` → `search` all `200`, and a planted fact ("Slack ticket escalation" workflow, integrations Slack + Zendesk) came back correctly on `search` — *"The integrations used were Slack and Zendesk."* Probe data dropped afterward (`DROP SCHEMA cognee CASCADE` + recreate), service restarted immediately per the lesson above, confirmed `healthy` again.

**Deployed:** `avry-user-dashboard` rebuilt and restarted (direct-file-patch of the two changed files onto the VPS checkout, matching this repo's established deploy pattern — its working tree already carries other sessions' uncommitted live patches, so `git pull` was correctly avoided here); new bundle confirmed to contain the new code (`grep` on the built `.next/server` output). Commit `355c5db` on `Aivory-hub88/avry-user-dashboard:main`.
