...
| 4. Tenant-identity bridge: Cerveau's native-bridge or gateway calls `cognee-http-server`'s HTTP API with `tenant_id = t_<user_id>.<agent_type>` | **Not started.** Confirmed why it can't be skipped: with `require_authentication: false` (this deployment's current setting), every caller collapses to one synthetic default user (`AuthenticatedUser::tenant_id: None` — `default_user_from_state`, `crates/http-server/src/auth/extractor.rs`). There is currently **no tenant isolation on this sidecar at all.** Do not point more than one tenant's data at it until this phase closes. |
...
| 6. Live verification: plant a graph-relationship fact, confirm it surfaces through search | **Mechanism proven** (§8) — single-tenant, direct-API smoke test, not yet through a real Cerveau tenant turn or the tool-call path (that's Phases 4–5). Re-run this same proof once 4–5 land, through the actual tenant path, before calling Phase 6 closed. |
...
**What this proves and what it doesn't.** The `pgvector` + `pggraph` build genuinely works end-to-end against `avry-postgres` — ingestion, chunking/embedding, entity/relationship extraction, and graph-aware retrieval all function, at 8 MB RSS. It does **not** yet prove tenant isolation (§6 Phase 4 — the sidecar currently has none, `require_authentication: false` collapses every caller to one identity) or that a real Cerveau agent turn can reach it (§6 Phase 5). Both remain open before any real tenant touches this.
