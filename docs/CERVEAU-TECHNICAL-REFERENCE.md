...
| **Lightpanda** | stdio MCP, `--block-private-networks` | Headless browsing, read-only by design. 22 read tools auto-approved; `session_*` variants beyond `session_list` deliberately untiered, therefore hard-denied |
| **Obscura** (added 2026-09-02) | stdio MCP, `--stealth`, private-network access left off (blocks loopback/RFC1918 by default, same posture as Lightpanda's flag) | Same `lightpanda-browsing` bundle as Lightpanda, all 4 agent types. Unlike Lightpanda, ships interactive tools (`click`/`fill`/`type`/`press_key`/`select_option`/`fill_form`) and cookie/storage mutation (`set_cookie`/`clear_cookies`/`set_storage_state`) plus `evaluate` (arbitrary JS) — all 10 explicitly tiered `irreversible` (hard floor, no autonomy mode exempts it), not left untiered like Lightpanda's excluded tools. The other 26 (navigate/snapshot/markdown/links/detectForms/screenshot/pdf/etc.) are `reversible` and auto-approved, mirroring Lightpanda's read tools |
...
**Lightpanda's `search` needed a real backend.** Without `BRAVE_API_KEY` or `TAVILY_API_KEY` the tool does not fail — it scrapes DuckDuckGo's HTML endpoint, which bot-blocks, and returns the anti-bot challenge page *as search results*. It is auto-approved on four agent types, so any turn reaching for web search was reasoning over a CAPTCHA page and pulling attacker-influenceable text into context on a path that looks like a search API. Fixed 2026-08-30 with a Tavily key in `/etc/zeroclaw-cerveau.env`; the `approval-slack-notify` timer now also watches the monthly credit balance and warns at 80% and 100%. `obscura__browser_search` was not checked against the same failure mode before shipping — worth verifying it doesn't hit the same DuckDuckGo-bot-block trap before relying on it.

**Obscura added as a second browsing toolkit, not a replacement (2026-09-02).** Reason: Lightpanda is read-only by construction (no click/fill/submit tools at all), and Obscura's stealth/anti-detection build adds real value on sites that block scrapers — while Lightpanda itself was separately found broken in production that same week (`Permission denied (os error 13)` on exec, still open, see `CERVEAU-TOOLKIT-EXPANSION-PLAN.md`'s incidental finding). Rather than debug that in isolation, Obscura went in alongside it in the same `lightpanda-browsing` bundle, giving every agent type a working fallback immediately.

Both toolkits' MCP servers, tool lists, and full risk-tier assignment were verified live against the real binaries (not assumed from docs) before rollout: `obscura mcp --stealth` was queried directly for its `tools/list` (36 tools, all of which line up with the table above), a live `browser_navigate` + `browser_markdown` call against `https://example.com` returned real content, and a call to `http://127.0.0.1:3100` confirmed the private-network block holds by default (`Access to private/internal IP address 127.0.0.1 is not allowed`) — the same SSRF posture as Lightpanda's `--block-private-networks`, just achieved by *omitting* Obscura's opt-in `--allow-private-network` flag rather than passing a block flag.

A new skill, `browser-tool-priority` (symlinked into all 4 agent types' skill bundles from one shared copy so the four never drift from each other), tells the agent: try Lightpanda first for anything read-only, fall back to the Obscura-equivalent tool automatically (with an explicit per-tool mapping table) if a Lightpanda call fails, and go straight to Obscura for anything Lightpanda structurally can't do (click/fill/submit/screenshot/PDF, or a site aggressive enough about bot-blocking that stealth mode matters). The switch is deliberately invisible to the end user — neither toolkit's name may be surfaced, per the existing `identity.md` rule against hinting at tools/providers/architecture.
