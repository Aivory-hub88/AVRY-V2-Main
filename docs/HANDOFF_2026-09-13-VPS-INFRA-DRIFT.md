...
- **Discovered and partially fixed:** the VPS directory `/home/ubuntu/AVRY-V2-Main` — which is the *real* deploy source for Traefik, the landing page, and most backend services — had ~300 tracked source files deleted from disk and never committed (the entire Next.js `src/`, `public/`, and root config for `avry-website`). **This has been restored and the live container cut over to a fresh build from it** — see §5. The rest of the drift (uncommitted `docker-compose.prod.yml`, `.gitmodules`/`traefik` confusion, a live secrets-exposure risk, months of other backup-file clutter) is still open — see §3.
- **Not done:** any git reconciliation of that directory (i.e., actually committing the restored files or the rest of the drift). Deliberately stopped short of it — see §4 for why and §6 for what's recommended instead.
...
## 5. Update — source restored and cut over, 2026-09-13 (same session, later)

The missing `avry-website` source (§3) has been restored and the live container cut over to a rebuild from it. Sequence:

1. Confirmed the local Mac checkout's `frontend/frontend-nextjs/` is at the exact same commit as the VPS (`8576d17`) and has the full source tree intact.
2. Test-built it locally (Node 25, then verified again against `node:20-alpine` matching production) — clean build, 47 routes, no fatal errors, only benign Next 16 `themeColor` metadata deprecation warnings.
3. Packaged (`tar`, excluding `node_modules`/`.next`/`.git`) and transferred to a **scratch** directory on the VPS (`/tmp/avry-website-source-restore`), built and ran there in isolation (`avry-website:source-restore-test`, scratch port `18099`) — verified `HTTP 200` and correct rendering — **before touching the live checkout at all**.
4. Only after that passed: `rsync`'d the specific missing files (`Dockerfile`, `.dockerignore`, `.eslintrc.json`, `.prettierrc`, `next.config.js`, `next-env.d.ts`, `package.json`, `package-lock.json`, `postcss.config.js`, `tailwind.config.ts`, `tsconfig.json`, `vitest.config.ts`, `src/`, `public/`) into `/home/ubuntu/AVRY-V2-Main/` — deliberately **not** the stray `frontend/` subdirectory that was also present in the source tarball (that's leftover pre-refactor dead code, superseded by root-level `src/` since commit `7ef294d`), and **not** `docker-compose.prod.yml` (already correct on the VPS, see §1). `git status` in that checkout now shows 0 remaining deletions.
5. Tagged the then-running image as a rollback point (`avry-website:rollback-pre-source-restore-20260913`) before touching anything live.
6. Built the real image via `docker compose -f docker-compose.prod.yml build avry-website` (picks up the correct production build args — `NEXT_PUBLIC_API_BASE_URL`, Turnstile site key, etc. — none of which the scratch test build had).
7. Cut over: `docker compose -f docker-compose.prod.yml up -d --no-deps avry-website`. Verified: container `healthy`, `https://aivory.uk/` and `/pricing` both `200`, clean startup log (`✓ Ready in 0ms`, no errors), a restored static asset (`Aivory_logo_2_2026.svg`) serves `200`, and the page title renders correctly.
8. Cleaned up the VPS scratch directory and tarball; kept both `avry-website:rollback-pre-source-restore-20260913` and `avry-website:source-restore-test` images for reference.

**What this does NOT resolve:** none of the rest of §3's findings (uncommitted `docker-compose.prod.yml` drift, the `.gitmodules`/`traefik` submodule confusion, the `.env.bak-*` secrets-exposure risk, the underlying "edit live + tag rollback images" workflow) were touched. `avry-website` can now be rebuilt from source again without failing, which was the specific, narrow problem this closed — the broader git/workflow drift in §3 is still open.

## 6. Recommended next steps (not done, for whoever picks this up)

1. Delete `.env.bak-pre-fx-margin-3pct-20260829` from the VPS (or move it somewhere outside any git working tree) — it's a live secrets-exposure risk sitting in a directory that periodically gets `git add`-ed to for other reasons.
2. Stop the "edit live on VPS + tag a Docker image as rollback" pattern going forward for this host. If a future session (AI-assisted or not) needs to make a landing-page tweak, prefer: edit the source in a proper checkout, commit, build, deploy — even if slower than the live-edit loop this directory has clearly been through for weeks. (Ironically, tonight's own restore used exactly this "build + tag rollback image" pattern once — deliberately, as the safest available option given the checkout's existing git drift, not as an endorsement of continuing it long-term.)
3. Decide, deliberately, whether/when to commit the now-restored source tree into checkout #1's own git history (Frntend-nxt) — it would resolve the `git status` drift for these specific files, but push credentials for that remote are not confirmed working from the VPS (per existing memory), and the directory has other unrelated uncommitted drift (`docker-compose.prod.yml`, `.gitmodules`/`traefik`) that a commit here shouldn't get entangled with without its own deliberate decision.
4. Separately and at leisure: reconcile `/home/ubuntu/avry-v2-main-src` (checkout #2) with current `origin/main` — this one is genuinely low-risk to fix properly since nothing except `avry-zeroclaw-daemon` depends on it, and its history is a clean prefix of the real monorepo's current history (its old HEAD, under a pre-rewrite SHA, corresponds content-wise to `origin/main`'s `091d979`).
