...
3. ~~**Tenant-identity source.**~~ **Decided 2026-09-14, in chat: identity follows the Cerveau *agent* the tenant is bound to, not a raw Odoo field.** Rather than deriving `X-Tenant-Id` from `res.company.id`/`res.users.id`, the admin explicitly sets two things in Settings: a **Cerveau Tenant ID** (still the "dedicated field, admin sets it during setup" option — no magic mapping) and a **Cerveau Agent** picker. The agent selection becomes the `X-Agent-Type` header. Implemented and verified 2026-09-14 in `models/res_config_settings.py` (`aivory_cerveau_tenant_id`, `aivory_cerveau_agent_type`), `views/res_config_settings_views.xml`, and `controllers/main.py`.
   ...
   **Deployed to production 2026-09-14, same day.** `backend/avry-backend` commit `ae1b1c2` pushed to `Aivory-hub88/avry-backend`; on `tencent-vps` at `/home/ubuntu/AVRY-V2-Main/backend/avry-backend` (which is **not** a clean checkout of that commit — it's a long-lived local working tree with real uncommitted hotfixes and dozens of `.bak-*` files, see `[[avry-backend-auth-postgres]]`/`[[white-box-memory-feature-and-avry-backend-drift]]`), the new `app/routes/agent_roster.py` file and its two-line registration in **both** `main.py` (the file the container actually runs — confirmed by an existing in-repo comment) and `app/main.py` (kept in sync manually, unused but matching the box's own established convention) were added by hand, mirroring exactly how the box's existing uncommitted `mcp_image_downloads` hotfix was wired, without touching or resolving the rest of that drift (notably an unexplained local removal of `agent_runs_router` from `app/main.py` — left alone since the reason for it isn't known). Rebuilt (`docker compose -f docker-compose.prod.yml build avry-backend`) and restarted; container healthy; verified live and public at `https://backend.aivory.id/api/v1/agent-roster` (200, correct 5-agent JSON). The VPS's `avry-backend` git working tree remains uncommitted/dirty exactly as it was found — this deploy did not attempt to reconcile it with origin.

   **Still not done:** pointing `frontend/avry-user-dashboard`'s `AGENT_DISPLAY_NAMES` and the backend's other `AGENT_TYPES` sets (`agent_profiles.py`, `telegram_service.py`) at this new roster instead of their own independent copies — those still exist untouched, so `agent_roster.py` is a fourth copy today, canonical only for this Odoo addon so far, not a true single source of truth network-wide.
