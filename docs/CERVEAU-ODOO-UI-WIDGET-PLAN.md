...
3. ~~**Tenant-identity source.**~~ **Decided 2026-09-14, in chat: identity follows the Cerveau *agent* the tenant is bound to, not a raw Odoo field.** Rather than deriving `X-Tenant-Id` from `res.company.id`/`res.users.id`, the admin explicitly sets two things in Settings: a **Cerveau Tenant ID** (still the "dedicated field, admin sets it during setup" option — no magic mapping) and a **Cerveau Agent** picker with the five real named agents from `frontend/avry-user-dashboard/lib/workspaceAccess.ts` (`AGENT_DISPLAY_NAMES`): Geno (`autonomous`, the generalist — default), Teo (`customer_service`), Lex (`leads_qualifier`), Finn (`finance_invoice_ops`), Ofira (`office_assistant`). The agent selection becomes the `X-Agent-Type` header. Implemented and verified 2026-09-14 in `models/res_config_settings.py` (`aivory_cerveau_tenant_id`, `aivory_cerveau_agent_type`), `views/res_config_settings_views.xml`, and `controllers/main.py`. **Note:** the `AGENT_TYPE_SELECTION` list in `res_config_settings.py` is a manually-kept copy of `AGENT_DISPLAY_NAMES` — there's no shared package between this addon and the dashboard repo, so a future agent rename needs updating in both places.
