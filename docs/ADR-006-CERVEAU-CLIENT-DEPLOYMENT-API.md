...
**A live end-to-end test the same day (registered a real throwaway account, created a real key, sent a real message — see `CERVEAU-STATUS.md`) surfaced that this fail-open path was silently *always* taken**: `cache.py`'s Redis client had been hardcoded to `host='localhost'` with no password since the file was written, and production Redis is a separate container reachable only by name (`redis`) with a password — `localhost` inside `avry-backend`'s own container has never had anything listening there. The rate limiter therefore never actually enforced anything; it just always failed open, silently, since before this feature existed. Fixed in the same round (`REDIS_HOST`/`REDIS_PORT`/`REDIS_PASSWORD` env-driven, defaults unchanged for local dev) and re-verified live: a real request produced a real `rate_limit:agent_api_key:{id}` counter in Redis afterward.
