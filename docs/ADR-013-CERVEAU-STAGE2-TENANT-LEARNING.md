...
- **Phase 2:** Stage 1 (synthesis) + the three kanban tools. Strictly propose-only — findings and draft skill deltas land for human review; nothing auto-ships. **Cross-repo prerequisite, since resolved:** this phase would have needed avry-backend's Agent Activity store fixed off flat-file JSON before wiring skill-insight notifications through it (the same path `task_ledger.rs`'s `notify_agent_action` uses for operational tasks) — the concurrent Agent Task Ledger work (2026-09-12, `CERVEAU-STATUS.md`) migrated `agent_actions` to a real Postgres table the same day this ADR was written, so this blocker no longer applies. Confirm the migration is still in place before relying on it if much time has passed.
