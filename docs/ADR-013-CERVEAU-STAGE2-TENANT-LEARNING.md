...
**Stage 2 — Kanban.** A new ledger, deliberately mirroring the **design principles** of the existing Agent Task Ledger (`crates/zeroclaw-memory/src/task_ledger.rs` + `crates/zeroclaw-tools/src/task_ledger.rs`) rather than sharing its table — the two have different lifecycles and consumers, and folding skill-insight rows into `agent_tasks` would conflate a tenant-support signal with the existing `avry-backend` task-notification consumer (`notify_agent_action`, `task_ledger.rs:196-240`) that only expects operational tasks. What *is* reused, line for line:
  ...
  **Deliberate divergence from `agent_tasks`:** the task ledger never deletes a row — `done` is terminal, and the row itself is the audit trail. `skill_insights` does the opposite once a row is fully resolved (shipped into a skill, or explicitly rejected as not-worth-solving): **delete the row.** An insight is working data for reaching a skill, not a permanent record in its own right, and unlike operational tasks (bounded by how much a human does), insight volume is bounded by how much tenant traffic exists — left to accumulate forever with `agent_tasks`'s philosophy, this table grows unbounded in the one direction Cerveau has repeatedly had to walk back for other subsystems (ADR-004's memory-lifecycle 650GB/yr warning is the same shape of mistake). The provenance that would have justified keeping the row instead moves to the *skill* it produced (§3 Stage 3) — a skill is a permanent record, an insight that fed it is not.

**Stage 3 — Distillation → new skill.** When a finding reaches a threshold status, trigger the *existing* `skills/review.rs` mechanism (already audited, rollback-tested, cooldown-limited) — but sourced from a `skill_insights` row instead of mid-conversation drift, and writing to the **per-tenant DB overlay** from §4 instead of the shared host workspace tree. The overlay row carries a compact `origin` field (source insight's title/description, synthesized case summary, timestamp) copied over at the moment of shipping — this is the skill's permanent provenance record: *where it came from and what case it solves*, readable without the `skill_insights` row still existing. Once that `origin` field is written and the overlay row is confirmed live, the source `skill_insights` row is deleted (see Stage 2's divergence note above) rather than moved to `status = 'distilled'` and kept.
...
- **A genuinely per-tenant skill overlay** — a DB table (tenant_id + agent_type + slug + content + version + tier + `origin`), not a filesystem tree. This is new: nothing in Cerveau today stores skill *content* anywhere but the filesystem, so `skill_manage.rs`'s write path needs a second branch (DB write when `current_tenant().is_some()`, current filesystem write otherwise), and the filesystem-native safety net (`safe_skill_dir`, `post_mutation_audit`, mtime-based cooldown in `improver.rs:26-59`) needs a DB-row equivalent (versioned rows, an audit check before the row is considered live, a timestamp-column cooldown). The `origin` column is the skill's only remaining link back to the insight(s) that produced it, once those rows are deleted per §3 Stage 2/3 — it needs to be a real, human-readable text field written at ship time, not a foreign key into a table that may no longer have the row.
...
- **Phase 2:** Stage 1 (synthesis) + the three kanban tools. Strictly propose-only — findings and draft skill deltas land for human review; nothing auto-ships. **Cross-repo prerequisite if this phase notifies a human via the dashboard** (the same way `task_ledger.rs`'s `notify_agent_action` does for operational tasks): avry-backend's Agent Activity store is flat-file JSON today — every read loads every record into memory (confirmed in the original Agent Task Ledger PRD, since built). Operational-task volume has been fine there; skill-insight volume is tied to tenant traffic, not human-initiated actions, and is a plausible way to actually hit that ceiling. Fix avry-backend's agent-actions storage to a real table *before* wiring skill-insight notifications through it — this is a dependency on another repo's fix, not something this ADR's Phase 2 can do by itself, and should be tracked as a blocking checklist item rather than assumed.
