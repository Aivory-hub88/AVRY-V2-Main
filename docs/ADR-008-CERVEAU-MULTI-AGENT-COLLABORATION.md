...
**⚠️ Addendum 2026-09-06 — the "no `[agents.customer_service]` section at all" claim above is no longer universally true.** A separate initiative ("give each Aivory product agent_type its own real Cerveau identity", see `CERVEAU-STATUS.md`'s 2026-09-06 entry) added `[agents.finance_invoice_ops]` as a pilot — a real, same-named, enabled entry that a tenant's own `X-Agent-Type` now resolves to directly. `handle_webhook_skills` was updated in the same push to resolve through `Config::resolved_runtime_agent_alias_for_tenant_type` instead of the type-blind fallback, so this route's own behavior is already correct for the migrated type; only the design-rationale sentence above is now stale for `finance_invoice_ops` specifically (still accurate for every other, unmigrated type).
